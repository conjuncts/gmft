...
try:
    ...
    from gmft.formatters.img2table import Img2TableFormatter
...
def test_i2t_pdf_compat(doc_tiny):
    ...
    extracted_tables = i2tdoc.extract_tables(
        ocr=None,
        implicit_rows=False,
    ...
    assert (
        page0tables[0].df.to_csv(index=False, sep="\t", lineterminator="\n")
        == """0	1	2
...
def test_i2t_detect(doc_tiny):
    ...
    assert len(cts) == 1
    assert (
        cts[0].df().to_csv(index=False, sep="\t", lineterminator="\n")
        == """0	1	2
Name	Celsius	Fahrenheit
Water Freezing Point	0	32
Water Boiling Point	100	212
