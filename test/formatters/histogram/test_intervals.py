...
def test_histogram_4():
    ...
    assert hist.sorted_points == expected_points, (
        f"Expected {expected_points}, got {hist.sorted_points}"
    )
...
def test_histogram_5():
    ...
    for point, expected_freq in test_points:
        assert hist.frequency(point) == expected_freq, (
            f"Expected frequency {expected_freq} at point {point}, got {hist.frequency(point)}"
        )
...
def test_histogram_6():
    ...
    assert hist.frequency(2) == 1, (
        "Point at interval boundary should belong to second interval only"
    )
...
def test_histogram_empty():
    ...
    assert hist.frequency(1) == 0, (
        "Intervals [1, 1) have measure 0 and should have no effect"
    )
...
def test_histogram_8():
    ...
    for interval in intervals:
        ...
        max_freq += 1
        assert max(point[1] for point in hist.sorted_points) == max_freq, (
            f"Maximum frequency should be {max_freq}"
        )

    # Test 9: Binary search edge cases
    hist = IntervalHistogram()
    hist.append((1, 5))
    assert hist.get_index_before_or_equal(0) == -1, (
        "Should return -1 for point before all intervals"
    )
    assert hist.get_index_before_or_equal(6) == 1, (
        "Should return last index for point after all intervals"
    )
    assert hist.get_index_after(6) == 2, (
        "Should return length for point after all intervals"
    )
